...
def compute_kl_reverse_process(x_start,
    ...
    p_t = p_forward(
        ...
        return_x0=predict_x0 and hybrid_lambda >= 0.0,  # raw: '> 0.0'
        return_logits=log_space,
        transition_probs=transition_probs,
        step_size=step_size,
        word_freq_logits=word_freq_logits
    ...
    if predict_x0 and hybrid_lambda >= 0.0:  # raw: '> 0.0'
        p_t, p_0 = p_t
        if log_space:
...
def discrete_diffusion_elbo(
    ...
    def elbo_body_fn(state, _):
        ...
        return State(
            ...
            recover=metrics_dict['recover'],
            loss=metrics_dict['loss'],
            ce_loss=metrics_dict['cross_entropy_loss']
        ), None
    ...
    return {
        ...
        "recover": final_state.recover,
        "loss": final_state.loss,
        "cross_entropy_loss": final_state.ce_loss
    }
...
def discrete_diffusion_predict_fn(
  ...
  """
    ...
    ##########################################################################
    # We modify the init of x to generate different sequences
    torch.manual_seed(time.time())
    x = torch.randint(0, diffusion.word_freq.shape[0], shape, device=device)
    ##########################################################################
    # pdb.set_trace()
