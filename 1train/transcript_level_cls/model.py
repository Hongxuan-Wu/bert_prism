...
class TranscriptLevelCls(nn.Module):
    def __init__(self, config):
        super(TranscriptLevelCls, self).__init__()
        ...
        if config['num_class'] == 3:
            self.classification = Classifier(config, 768, 3)
        elif config['num_class'] == 4:
            self.classification = Classifier(config, 768, 4)
